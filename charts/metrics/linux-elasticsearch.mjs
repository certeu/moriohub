export default {
  cluster_stats: (params) => {
    const noop = (val) => val
    const formatNumber = params?.formatNumber || noop
    const formatBytes = params?.formatNumber || noop
    const charts = [
      // Docs
      {
        id: 'docs',
    ...
    if (!params) {
      const info = {}
      for (const chart of charts) info[chart.id] = chart.title
      return info
    }

    return params.lineChart(charts, params.data)
  },

  cluster_status: (params) => {
    if (!params) return { status: 'Elasticsearch Cluster Status' }

    const { data, templates, clone, formatBytes, formatNumber, chartGradient } = params
