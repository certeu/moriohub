...
`,
  settings: {
    ...
    hostCap: {
      dflt: 150,
      title: 'Health check cache per-host entry limit',
      type: 'number',
    },
    eventify: {
      dflt: true,
...
/*
 * This handler is for healtcheck data.
 *
...
function handler (params) {
   ...
   */
  const summary = healthcheckSummary(params)

  /*
   * Update the cache
   */
  if (settings.cache) tools.cache.healthcheck(summary, settings)
  ...
  if (
    ...
    md_title: `Health check failed: ${tools.link.md.healthcheck(summary.id, summary.name)}`,
  ...
  if (summary.dbce !== undefined) {
    const { certificate_days = 10 } = settings
    if (
